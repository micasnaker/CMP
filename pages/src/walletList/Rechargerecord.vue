<template>
	...
	<view class="box">
		...
		<view style="margin: 20upx;" v-for="(item,index) in data.list" key="index">
			<view style="color: #349cfe;font-size: 30upx;">{{item.type==1?'余额充值':'钻石充值'}}({{item.status==0?'申请中':item.status>0?'已完成':'已驳回'}})</view>
				<view style="float: right;color: #fff;">{{item.num}}</view>
...
<script>
	...
	export default{
		...
		methods:{
			...
			// 		page:1
			// 	}
			// 	let res = await service.httpRequest('chongzhi_log',query)
			// 	this.data = res.data
			// 	
